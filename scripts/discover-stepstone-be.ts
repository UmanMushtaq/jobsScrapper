...
const KEYWORDS = ['api', 'search', 'job', 'vacancy', 'result', 'offer', 'listing', 'graphql', 'rest', 'xhr'];

(async () => {
  const browser = await chromium.launch({ headless: true });

  for (const query of QUERIES) {
    const url = `https://www.stepstone.be/jobs/${encodeURIComponent(query)}`;
    console.log(`\n${'='.repeat(60)}`);
