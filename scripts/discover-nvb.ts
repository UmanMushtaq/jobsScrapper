...
const KEYWORDS = ['api', 'search', 'job', 'vacancy', 'result', 'offer', 'listing', 'graphql', 'rest', 'xhr', 'nvb', 'vacature'];

(async () => {
  const browser = await chromium.launch({ headless: true });

  for (const query of QUERIES) {
    const url = `https://www.nvb.nl/vacatures/?q=${encodeURIComponent(query)}`;
    console.log(`\n${'='.repeat(60)}`);
