...
const KEYWORDS = ['api', 'search', 'job', 'vacancy', 'result', 'offer', 'listing', 'graphql', 'rest', 'xhr', 'jobat'];

(async () => {
  const browser = await chromium.launch({ headless: true });

  for (const query of QUERIES) {
    const url = `https://www.jobat.be/nl/jobs?q=${encodeURIComponent(query)}`;
    console.log(`\n${'='.repeat(60)}`);
