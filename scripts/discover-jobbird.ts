...
const KEYWORDS = ['api', 'search', 'job', 'vacancy', 'result', 'offer', 'listing', 'graphql', 'rest', 'xhr', 'jobbird', 'vacature'];

(async () => {
  const browser = await chromium.launch({ headless: true });

  for (const query of QUERIES) {
    const url = `https://www.jobbird.com/nl/vacature?s=${encodeURIComponent(query)}`;
    console.log(`\n${'='.repeat(60)}`);
