...
export class JobsLuSource implements JobSource {
  name = SOURCE;
  priority = 4;
    ...
    for (const query of SEARCH_QUERIES) {
      try {
        ...
        if (!msg.includes('ECONNREFUSED') && !msg.includes('ETIMEDOUT')) {
          console.error(`[jobs-lu] error for "${query}": ${msg}`);
        }
      }
    }

    if (jobs.size === 0) console.log(`[jobs-lu] 0 jobs — may be blocked`);
    else console.log(`[jobs-lu] ${jobs.size} unique jobs fetched`);
    return Array.from(jobs.values());
  }
}

async function fetchPage(query: string, cutoff: number): Promise<JobPosting[]> {
  const targetUrl = `${BASE_URL}/en/jobs/?query=${encodeURIComponent(query)}`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[jobs-lu] blocked ${res.status} for "${query}"`);
    return [];
  }
