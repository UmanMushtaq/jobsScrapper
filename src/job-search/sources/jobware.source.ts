    ...
    for (const query of SEARCH_QUERIES) {
      try {
        ...
        if (fetched.preview && !loggedPreview) {
          console.log(`[jobware] HTML preview for "${query}": ${fetched.preview}`);
          loggedPreview = true;
        }
        ...
        if (!msg.includes('ECONNREFUSED') && !msg.includes('ETIMEDOUT')) {
          console.error(`[jobware] error for "${query}": ${msg}`);
        }
      }
    }

    if (jobs.size === 0) console.log(`[jobware] 0 jobs — may be blocked or structure changed`);
    else console.log(`[jobware] ${jobs.size} unique jobs fetched`);
...
async function fetchPage(query: string, cutoff: number, capturePreview: boolean): Promise<{ jobs: JobPosting[]; preview: string | null }> {
  const url = `${BASE_URL}/suche/?suche=${encodeURIComponent(query)}`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[jobware] blocked ${res.status} for "${query}"`);
