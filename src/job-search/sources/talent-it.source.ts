...
const SOURCE = 'talent.it';
const BASE_URL = 'https://www.talent.it';
...
export class TalentItSource implements JobSource {
  name = SOURCE;
  priority = 4;
    ...
    for (const query of SEARCH_QUERIES) {
      try {
        ...
        if (!msg.includes('ECONNREFUSED') && !msg.includes('ETIMEDOUT')) {
          console.error(`[talent-it] error for "${query}": ${msg}`);
        }
      }
    }

    if (jobs.size === 0) console.log(`[talent-it] 0 jobs — may be blocked`);
    else console.log(`[talent-it] ${jobs.size} unique jobs fetched`);
    return Array.from(jobs.values());
  }
}

async function fetchPage(query: string, cutoff: number): Promise<JobPosting[]> {
  const targetUrl = `${BASE_URL}/offerte-lavoro?q=${encodeURIComponent(query)}`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[talent-it] blocked ${res.status} for "${query}"`);
    return [];
  }
