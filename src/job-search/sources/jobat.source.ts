...
export class JobatSource implements JobSource {
  name = SOURCE;
  priority = 4;
    ...
    for (const query of SEARCH_QUERIES) {
      try {
        ...
        if (fetched.preview && !loggedPreview) {
          console.log(`[jobat] HTML preview for "${query}": ${fetched.preview}`);
          loggedPreview = true;
        }
        ...
        if (!msg.includes('ECONNREFUSED') && !msg.includes('ETIMEDOUT')) {
          console.error(`[jobat] error for "${query}": ${msg}`);
        }
      }
    }

    if (jobs.size === 0) console.log(`[jobat] 0 jobs — may be blocked or structure changed`);
    else console.log(`[jobat] ${jobs.size} unique jobs fetched`);
...
async function fetchPage(query: string, cutoff: number, capturePreview: boolean): Promise<{ jobs: JobPosting[]; preview: string | null }> {
  const url = `${BASE_URL}/en/jobs?keywords=${encodeURIComponent(query)}`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[jobat] blocked ${res.status} for "${query}"`);
  ...
  const jobs = rawJobs
    ...
    .map((j) => mapRawJob(j, SOURCE, 4, 'BE', 'Belgium', BASE_URL))
