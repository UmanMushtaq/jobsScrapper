...
export class XingJobsSource implements JobSource {
  name = SOURCE;
  priority = 4;
    ...
    for (const query of SEARCH_QUERIES) {
      try {
        ...
        if (!msg.includes('ECONNREFUSED') && !msg.includes('ETIMEDOUT')) {
          console.error(`[xing] error for "${query}": ${msg}`);
        }
      }
    }

    if (jobs.size === 0) console.log(`[xing] 0 jobs — may be blocked`);
    else console.log(`[xing] ${jobs.size} unique jobs fetched`);
    return Array.from(jobs.values());
  }
}

async function fetchPage(query: string, cutoff: number): Promise<JobPosting[]> {
  const targetUrl = `${BASE_URL}/jobs/search?keywords=${encodeURIComponent(query)}&location=Germany`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[xing] blocked ${res.status} for "${query}"`);
    return [];
  }
