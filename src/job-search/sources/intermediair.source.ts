...
const SOURCE = 'intermediair.nl';
const BASE_URL = 'https://www.intermediair.nl';
...
export class IntermediairSource implements JobSource {
  name = SOURCE;
  priority = 4;
    ...
    for (const query of SEARCH_QUERIES) {
      try {
        ...
        if (!msg.includes('ECONNREFUSED') && !msg.includes('ETIMEDOUT')) {
          console.error(`[intermediair] error for "${query}": ${msg}`);
        }
      }
    }

    if (jobs.size === 0) console.log(`[intermediair] 0 jobs — may be blocked`);
    else console.log(`[intermediair] ${jobs.size} unique jobs fetched`);
    return Array.from(jobs.values());
  }
}

async function fetchPage(query: string, cutoff: number): Promise<JobPosting[]> {
  const targetUrl = `${BASE_URL}/vacatures?zoekterm=${encodeURIComponent(query)}`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[intermediair] blocked ${res.status} for "${query}"`);
    return [];
  }
