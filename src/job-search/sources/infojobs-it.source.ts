...
const SOURCE = 'infojobs.it';
const BASE_URL = 'https://www.infojobs.it';
...
export class InfoJobsItSource implements JobSource {
  name = SOURCE;
  priority = 4;
    ...
    for (const query of SEARCH_QUERIES) {
      try {
        ...
        if (!msg.includes('ECONNREFUSED') && !msg.includes('ETIMEDOUT')) {
          console.error(`[infojobs-it] error for "${query}": ${msg}`);
        }
      }
    }

    if (jobs.size === 0) console.log(`[infojobs-it] 0 jobs — may be blocked`);
    else console.log(`[infojobs-it] ${jobs.size} unique jobs fetched`);
    return Array.from(jobs.values());
  }
}

async function fetchPage(query: string, cutoff: number): Promise<JobPosting[]> {
  const targetUrl = `${BASE_URL}/offerte-lavoro/?keyword=${encodeURIComponent(query)}`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[infojobs-it] blocked ${res.status} for "${query}"`);
    return [];
  }
  ...
  return rawJobs
    ...
    .map((j) => mapRawJob(j, SOURCE, 4, 'IT', 'Italy', BASE_URL))
