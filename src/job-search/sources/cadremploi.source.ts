...
export class CadremploiSource implements JobSource {
  ...
  async fetch(_queries: string[], settings: SearchSettings): Promise<JobPosting[]> {
    ...
    if (jobs.size === 0) console.log(`[cadremploi] 0 jobs — may be blocked`);
...
async function fetchPage(query: string, cutoff: number): Promise<JobPosting[]> {
  const targetUrl = `${BASE_URL}/emploi/liste_offres.html?kw=${encodeURIComponent(query)}&lieux=france`;
  ...
  if (res.status === 403 || res.status === 429) {
    console.log(`[cadremploi] blocked ${res.status} for "${query}"`);
    return [];
  }
