...
const SYSTEM_INSTRUCTION = (name: string, expYears: number, cvText: string, workMode: string, countryCode: string | null, visaContext: string, statusLine: string, jobSource = '') =>
  ...
  `    FR SHORTAGE OCCUPATION SENTENCE (additive, FR jobs only): if countryCode="${countryCode ?? 'null'}" is "FR", add ONE additional short sentence directly after the statusLine above, stating that the candidate's profession (software engineer) is on France's official shortage occupation list (metier en tension), which simplifies hiring formalities on the employer side. Suggested phrasing: "My profession is on France's official shortage occupation list, which simplifies the hiring formalities on the employer side." This sentence is ONLY for FR jobs — never include it for jobs in any other country, and never for remote jobs at non-French companies.\n` +
  `      STRICT GUARDRAILS for this sentence: it must NEVER mention visa expiry dates, recepisse, carte de sejour, changement de statut, or any status-change or renewal details. It refers only to the shortage-occupation/hiring-formality fact, nothing about the candidate's personal visa timeline, expiry, or status. The statusLine above must remain exactly as given, word for word, unmodified by this addition — this sentence is a separate, additional line, never a replacement.\n` +
